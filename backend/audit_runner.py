#!/usr/bin/env python3
"""Run council audit - standalone version."""
import os
import json
import time
import re
...
ROOT = Path("/mnt/c/Users/USER/Downloads/b_a6LznsoAKUT-1774336963705/backend")
os.chdir(ROOT)
findings: list = []
guild_scores: dict = {}
...
print(f"{DIM}  Time: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
...
em_content_str = em_content if em_exists else ""
check(bool(re.search(r"xadd|XADD", em_content_str)),
...
all_src = (py_files("backend") + py_files("workers") +
           list(ROOT.glob("*.py")) +
           (list((ROOT / "frontend").rglob("*.ts")) if exists("frontend") else []))
...
# ═══════════════════════════════════════════════════════════════
g7 = GuildScore("Code Quality", "All Council Members")
guild_scores["g7"] = g7
...
GUILD_LABELS = [
    ...
    ">> Code Quality   (All Council Members)",
]

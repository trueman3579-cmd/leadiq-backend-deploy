"""
collectors/cutshort.py — CutShort job scraper.

Curated tech job platform: https://cutshort.com/
"""
...
CUTSHORT_BASE_URL = "https://cutshort.com"


class CutshortCollector(BaseCollector):
    source = "cutshort"
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("cutshort_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("cutshort_search_failed", keyword=keyword, error=str(e))
        logger.info("CutshortCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{CUTSHORT_BASE_URL}/jobs/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("cutshort_fetch_failed", url=url, status=result.status, error=result.error)
            return []

        soup = BeautifulSoup(result.data.get("text", ""), "html.parser")
        cards = soup.select(
            "div.job-card, div[class*='job'], div[class*='card'], "
            "section[class*='job'], div[class*='JobCard']"
        )
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            title_el = card.select_one(
                ...
                "[class*='jobTitle'], [class*='job-title'], [class*='role']"
            )
            company_el = card.select_one(
                "span[class*='company'], div[class*='company'], "
                "[class*='comp'], [class*='employer'], [class*='org']"
            )
            ...
            link = f"{CUTSHORT_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("cutshort_card_parse_failed", error=str(exc))
            return None

"""
collectors/hirect.py — Hirect job scraper.

Direct-hire job platform: https://www.hirect.in/
"""
...
HIRECT_BASE_URL = "https://www.hirect.in"


class HirectCollector(BaseCollector):
    source = "hirect"
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("hirect_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("hirect_search_failed", keyword=keyword, error=str(e))
        logger.info("HirectCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{HIRECT_BASE_URL}/job-search/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("hirect_fetch_failed", url=url, status=result.status, error=result.error)
            return []
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{HIRECT_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("hirect_card_parse_failed", error=str(exc))
            return None

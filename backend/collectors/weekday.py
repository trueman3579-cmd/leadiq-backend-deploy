"""
collectors/weekday.py — Weekday job scraper.

Curated tech hiring platform: https://www.weekday.work/
"""
...
WEEKDAY_BASE_URL = "https://www.weekday.work"


class WeekdayCollector(BaseCollector):
    source = "weekday"
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("weekday_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("weekday_search_failed", keyword=keyword, error=str(e))
        logger.info("WeekdayCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{WEEKDAY_BASE_URL}/jobs/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("weekday_fetch_failed", url=url, status=result.status, error=result.error)
            return []

        soup = BeautifulSoup(result.data.get("text", ""), "html.parser")
        cards = soup.select(
            "div.job-card, div[class*='job'], div[class*='card'], "
            "section[class*='job'], div[class*='JobCard'], a[href*='/job/']"
        )
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{WEEKDAY_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("weekday_card_parse_failed", error=str(exc))
            return None

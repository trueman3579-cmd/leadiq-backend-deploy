"""
collectors/iimjobs.py — IIMJobs scraper.

India's premium management job platform: https://www.iimjobs.com/
"""
...
class IIMJobsCollector(BaseCollector):
    source = "iimjobs"
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("iimjobs_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("iimjobs_search_failed", keyword=keyword, error=str(e))
        logger.info("IIMJobsCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{IIMJOBS_BASE_URL}/search/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("iimjobs_fetch_failed", url=url, status=result.status, error=result.error)
            return []
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{IIMJOBS_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("iimjobs_card_parse_failed", error=str(exc))
            return None

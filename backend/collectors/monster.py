"""
collectors/monster.py — Monster India job scraper.

India's legacy job board: https://www.monsterindia.com/
"""
...
MONSTER_BASE_URL = "https://www.monsterindia.com"


class MonsterCollector(BaseCollector):
    source = "monster"
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("monster_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("monster_search_failed", keyword=keyword, error=str(e))
        logger.info("MonsterCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{MONSTER_BASE_URL}/search/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("monster_fetch_failed", url=url, status=result.status, error=result.error)
            return []

        soup = BeautifulSoup(result.data.get("text", ""), "html.parser")
        cards = soup.select(
            "div.job-card, div.card, div[class*='job-card'], "
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            title_el = card.select_one(
                "h3[class*='title'], h2[class*='title'], .job-title, "
                "[class*='jobTitle'], a[class*='title']"
            )
            company_el = card.select_one(
                "span[class*='company'], div[class*='company'], "
                "[class*='compName'], [class*='company-name']"
            )
            ...
            link = f"{MONSTER_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("monster_card_parse_failed", error=str(exc))
            return None

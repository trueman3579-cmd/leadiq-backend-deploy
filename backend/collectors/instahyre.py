"""
collectors/instahyre.py — Instahyre job scraper.

AI-driven job platform: https://www.instahyre.com/
"""
...
class InstahyreCollector(BaseCollector):
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("instahyre_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("instahyre_search_failed", keyword=keyword, error=str(e))
        logger.info("InstahyreCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{INSTAHYRE_BASE_URL}/jobs/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("instahyre_fetch_failed", url=url, status=result.status, error=result.error)
            return []
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{INSTAHYRE_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("instahyre_card_parse_failed", error=str(exc))
            return None

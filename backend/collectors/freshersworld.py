"""
collectors/freshersworld.py — Freshersworld job scraper.

India's leading fresher job platform: https://www.freshersworld.com/
"""
...
FRESHERWORLD_BASE_URL = "https://www.freshersworld.com"
...
class FreshersworldCollector(BaseCollector):
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("freshersworld_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("freshersworld_search_failed", keyword=keyword, error=str(e))
        logger.info("FreshersworldCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{FRESHERWORLD_BASE_URL}/jobs/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("freshersworld_fetch_failed", url=url, status=result.status, error=result.error)
            return []
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{FRESHERWORLD_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("freshersworld_card_parse_failed", error=str(exc))
            return None

"""
collectors/foundit.py — Foundit (formerly Monster) job scraper.

India's recruitment platform: https://www.foundit.in/
"""
...
class FounditCollector(BaseCollector):
    ...
    async def collect(self) -> list[RawPost]:
        ...
        for keyword in self._keywords:
            try:
                ...
                logger.info("foundit_search_complete", keyword=keyword, jobs_found=len(jobs))
            except Exception as e:
                logger.warning("foundit_search_failed", keyword=keyword, error=str(e))
        logger.info("FounditCollector fetched %d jobs", len(all_jobs))
        return all_jobs

    async def _scrape(self, keyword: str) -> list[RawPost]:
        url = f"{FOUNDIT_BASE_URL}/search/{keyword}"
        result = await self._adapter.fetch(url)
        if not result.is_success():
            logger.warning("foundit_fetch_failed", url=url, status=result.status, error=result.error)
            return []
        ...
        cards = soup.select(
            ...
            "div[class*='jobCard'], li[class*='job']"
        )
    ...
    def _parse_card(self, card: Any) -> RawPost | None:
        try:
            ...
            link = f"{FOUNDIT_BASE_URL}{href}" if href and href.startswith("/") else href
            ...
            logger.warning("foundit_card_parse_failed", error=str(exc))
            return None
